...
def BagofVisualWorlds(folder):
	...
	outfile=open(os.path.join(folder,"output\File.txt"), 'w')
	...
	means = K_MeansClustering(os.path.join(folder,"output\File.txt"))
...
def generateFeatureVectors(folder):
    ...
    return BagofVisualWorlds(os.path.join(folder,"output"))
